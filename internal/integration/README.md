...
| ingestion_confirm_test.go | Discoverable real-MySQL User and collector confirmation tests: issue #83 behavior plus discovered/ownerless collector create, machine audit attribution, server fingerprint validation, empty complete/incomplete/failed terminal scans, third-omission list/detail Missing projection with source/principal and stable time, recovery, exact/conflicting retry, never-archive, and whole-transaction rollback |
