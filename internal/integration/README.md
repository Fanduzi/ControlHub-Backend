...
| ingestion_confirm_test.go | Discoverable real-MySQL User and collector tests: real-router machine-authenticated empty CSV/JSON preview-to-confirm and ledger persistence, issue #83 behavior plus discovered/ownerless create, machine audit attribution, server fingerprint validation, complete/incomplete/failed scans, third-omission list/detail Missing projection with source/principal and stable time, recovery, exact/conflicting retry, never-archive, and whole-transaction rollback |
