...
| query_execution_service.go | Governed query execution and Phase 38S result paging with per-page access and disclosure; one private persistence implementation records every post-target Evidence-Bearing Query Attempt across ordinary, paged, template, and related-record navigation paths as an atomic Execution Evidence Pair in the detached two-second window (Issues #34/#35/#36/#61); Preflight and Apply disclosure blocks both publish `query_result_disclosure_blocked` (Issue #48); exposes `QueryEvidencePersistenceFailures` |
...
| query_execution_service_test.go | Query execution service tests, including governed paging, disclosure, persistence and cancellation durability (Issue #35), plus Apply-path exclusive `ErrQueryDisclosureBlocked` (Issue #48) |
