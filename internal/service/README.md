...
| query_execution_service.go | Governed query execution with validated user-or-machine identity and truthful evidence; one private persistence implementation records every post-target attempt across ordinary, paged, user-only template, and user-only navigation paths as an atomic Execution Evidence Pair in the detached two-second window; preserves cancellation, disclosure, and persistence-failure behavior |
...
| query_execution_service_test.go | Query execution service tests, including machine identity across every terminal outcome, governed paging, disclosure, atomic persistence, and cancellation durability |
