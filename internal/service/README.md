...
| ingestion_preview.go | Strict bounded CSV/JSON parsing, format-independent empty fingerprints, exact-identity ingestion classification, additive observed-field diffs, drift fingerprinting, collector scan-conflict sentinel, validation helpers, and repository-backed User/collector confirmation delegation including empty terminal scans |
| ingestion_preview_test.go | Parser equivalence/guard, empty terminal scan/fingerprint, pure preview precedence, immutable-type conflict, additive observed diff, manual-override exclusion, and collector metadata delegation tests |
...
| query_execution_service.go | Governed query execution with validated user-or-machine identity and truthful evidence; successful User attempts persist owner-private full SQL and expose it only through exact owner/target/success lookup; one private persistence implementation records every post-target attempt as an atomic Execution Evidence Pair |
...
| query_execution_service_test.go | Query execution service tests, including successful-User full SQL, owner-only retrieval, machine/non-success omission, governed paging, disclosure, atomic persistence, and cancellation durability |
...
- `ParseIngestion` and `PreviewIngestion` provide the controlled issue #83 parsing and preview seam; immutable CI-type mismatches are conflicts and observed diffs contain only submitted fields; ordinary previews/User confirmations require at least one row, while `ConfirmCollectorIngestion` accepts an empty terminal receipt and delegates its reviewed fingerprint, collector principal, and normalized scan metadata to the repository
