...
| query_execution_service.go | Governed query execution and Phase 38S result paging with per-page access, disclosure, and atomically paired history+audit (`InsertExecutionWithAudit`, Issue #34) persisted in a fixed two-second Evidence Persistence Window detached from request cancellation/deadline, with client-cancellation evidence recorded as failed/query_canceled (Issue #35); FK related-record navigation classifies inspector-phase cancellation/deadline as terminal failed/timeout evidence (Issue #40); Preflight disclosure rejections wrap `ErrQueryDisclosureBlocked` with `ErrQueryNotAllowed`, and Apply-path classification returns `ErrQueryDisclosureBlocked` as the HTTP sentinel so both publish `query_result_disclosure_blocked` (Issue #48); exposes the `QueryEvidencePersistenceFailures` counter through the service layer |
...
| navigate_related_records_test.go | Related-record navigation service tests: governance, parameter binding, history/audit, error mapping including Apply-path exclusive ErrQueryDisclosureBlocked (Issue #48), and inspector-phase cancellation/deadline terminal evidence via the detached Evidence Persistence Window (Issue #40) |
| query_execution_service_test.go | Query execution service tests, including governed per-page access, disclosure, persistence guarantees, cancellation-durable terminal evidence via the detached two-second Evidence Persistence Window (Issue #35), and Apply-path exclusive ErrQueryDisclosureBlocked so HTTP cannot collapse a policy block into query_not_allowed (Issue #48) |
