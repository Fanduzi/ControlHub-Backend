...
func TestDisclosureProjectionResolveExecute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "literal-only SELECT returns plan with columns", statement: "SELECT 1", want: []ColumnProvenance{{OutputName: "column_1"}}},
		{name: "literal-only SELECT with alias", statement: "SELECT 1 AS id", want: []ColumnProvenance{{OutputName: "id"}}},
		{name: "literal-only SELECT multiple literals", statement: "SELECT 1, 'hello', NULL", want: []ColumnProvenance{
			{OutputName: "column_1"},
			{OutputName: "column_2"},
			{OutputName: "column_3"},
		}},
		{name: "literal-only SELECT with function rejected", statement: "SELECT NOW()", wantErr: true},
		{name: "literal-only SELECT with expression rejected", statement: "SELECT 1+1", wantErr: true},
		...
		{name: "literal-only SELECT with GROUP BY rejected", statement: "SELECT 1 GROUP BY 1", wantErr: true},
		{name: "FROM dual blocked", statement: "SELECT 1 FROM dual", wantErr: true},
		{name: "FROM dual with alias blocked", statement: "SELECT 1 AS n FROM dual", wantErr: true},
		{name: "FROM dual qualified blocked", statement: "SELECT 1 FROM test.dual", wantErr: true},
		{name: "FROM dual aliased blocked", statement: "SELECT 1 FROM dual d", wantErr: true},
		{name: "FROM dual with block comment blocked", statement: "SELECT 1 FROM /* comment */ dual", wantErr: true},
		{name: "FROM dual with line comment blocked", statement: "SELECT 1 FROM -- comment\ndual", wantErr: true},
		{name: "FROM dual in comment only allowed", statement: "SELECT 1 /* FROM dual */", want: []ColumnProvenance{{OutputName: "column_1"}}},
