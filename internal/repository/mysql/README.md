...
| File | Responsibility |
|------|---------------|
| resource_repository.go | Resource CRUD, governed identity and batched typed-profile/per-principal collector-presence reads, key-presence/exact-value label filters, latest-per-observer health evidence, effective-health derivation, fail-closed cluster member operational summaries, per-source observed/effective values, read-only validated bulk previews, atomic audited identity/manual-override/bulk mutations, and User/collector atomic ingestion confirmation including empty terminal receipts |
| resource_repository_operational_summary_test.go | SQL-level fail-closed coverage for cluster member rollup query/scan errors so ListResources/GetResource cannot omit member counts as an empty success |
...
| relation_repository.go | Relation queries plus atomic create/delete serialized by stable endpoint resource-row locks, effective-health relation/member projections, deterministically bounded topology relation/candidate reads, and one-row-per-candidate health selection |
| relation_repository_topology_test.go | SQL-level regression coverage for topology ordering, caller-owned relation/candidate row limits, and observation scans bounded to candidate count |
| audit_repository.go | Audit event queries (global and by resource), including nullable user/machine identity plus privacy-safe actor labels, target-resource environment filtering, principal-name search, pagination, and JSON field changes |
...
| query_execution_repository.go | Query credential metadata plus atomic identity-aware `InsertExecutionWithAudit`; full statements persist only for successful user executions, lists project only their presence, and SQL is readable only for an exact owner/user/success match |
| query_execution_repository_evidence_test.go | Atomic user/machine evidence, truthful actor and private-statement-availability projection, rollback, and fixed safe persistence-failure telemetry coverage |
...
- `QueryExecutionRepository.ListExecutions` — history metadata includes only internal full-statement availability, never the private SQL; the service combines it with the authenticated User identity before publishing restore eligibility
...
- `RelationRepository.ListTopologyCandidates` returns at most the caller's limit of unique environment-scoped Service, Database Cluster, Database Proxy, and effective warning/critical starts, ordered by candidate category then name/ID; for only those bounded candidates, SQL selects one observation per resource (fresh worst severity, then newest timestamp, then observer tie-break; otherwise newest stale) before the existing domain resolver applies manual-override precedence
