...
func TestAuthenticatedActorRejectsMissingBearerToken(t *testing.T) {
	...
	h := requireAuthenticatedActor(svc, cfg)(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestAuthenticatedActorStoresActorInContext(t *testing.T) {
	...
	var capturedOK bool
	h := requireAuthenticatedActor(svc, cfg)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestFreshQueryActorEnforcesFixedEightHourConstant(t *testing.T) {
	...
	h2 := requireFreshQueryActor(svc, cfg)(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Error("handler must not run for a token beyond the fixed 8h bound")
	}))
