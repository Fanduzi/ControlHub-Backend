...
| ingestion_handler.go | Bounded strict multipart CSV/JSON ingestion preview plus User confirmation or verified collector terminal confirmation (including empty scans) with required scan metadata and controlled retry conflict mapping |
...
| ingestion_handler_test.go | Admin multipart ingestion preview/User-confirm compatibility plus empty collector terminal scans, scan-metadata validation, propagation, and 409 conflict tests |
