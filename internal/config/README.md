...
- `Load() (Config, error)` — reads APP_PORT, DATABASE_DSN, JWT_SECRET from environment via `os.LookupEnv`; returns `ErrQueryExecutionTokenMaxAgeRejected` if the removed QUERY_EXECUTION_TOKEN_MAX_AGE env var is present (even if empty)
