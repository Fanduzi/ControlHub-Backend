...
| openapi_test.go | Validates schema, bounded key/key:value resource label filters, read-only collector presence, reachable empty collector preview and conditional terminal-confirm multipart metadata, bulk mutation, topology, pagination, executions, and the closed controlled-error enum |
