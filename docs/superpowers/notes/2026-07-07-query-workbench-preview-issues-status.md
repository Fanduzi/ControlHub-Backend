...
| - | ------------- | -------------- | --------------- |
| 1 | Query target selector is too weak: cramped dropdown, truncated text, no search, filters visually separated from target identity. | Partially improved in Phase 38C/38D; Phase 38E must turn this into a connection-navigation surface with grouped/searchable targets and filters inside the target navigation area. | Phase 38E |
...
| 4 | Governance and access panel is too large for its value. | Compact badges and tooltips started in Phase 38C. Phase 38E must finish the compression: blocker-first layout, details behind tooltip/disclosure, no large permanent education cards. | Phase 38E |
| 5 | Target facts are duplicated and occupy too much space: engine, environment, host, owner, language, readiness, cluster. | Partially compacted in Phase 38C/38D. Phase 38E must show active target identity once and remove duplicated facts from governance/schema areas. | Phase 38E |
...
| 7 | Worksheet editor is not IDE-like: no SQL highlighting, no formatting, no multiple worksheets. | Not in Phase 38E. This is intentionally deferred to a later SQL editor foundation phase. | Future Phase 38F candidate |
