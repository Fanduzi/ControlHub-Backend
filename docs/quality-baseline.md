...
Frontend Phase 38H branch `phase-38h-query-workbench-scalable-ia-reset` (commit `12f97ed`):
- Backend paged/searchable `GET /query-targets` with `q`, `page`, `pageSize`, `targetId`, `pageInfo`.
- Frontend consumes paged contract; connection navigator opens on demand.
- Two-region database IDE layout: collapsible left explorer, dominant editor/results.
- Inline governance: one blocker near editor, compact badges, details on demand.
- Credential admin paginated (25/50/100), credential status fan-out scoped to current page IDs.
- Credential edit via modal/drawer; mobile uses full-screen surface.
- Active target preserved across search/page via URL `targetId`.
- 850/850 unit tests passed. Lint 0 errors, 2 pre-existing warnings.
- Real E2E 37/37 passed against backend `:8080` + dedicated query MySQL fixture.
- No SQL guard changes, no DSN/password leak, no `actorUserId`, no credential edit controls on `/query`.
